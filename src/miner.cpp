...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("UTUMMiner : generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, NULL, pblock))
        return error("UTUMMiner : ProcessNewBlock, block not accepted");
...
{
    LogPrintf("UTUMMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("utum-miner");
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        ...
        if (fProofOfStake) {
            ...
            if (!pblock->SignBlock(*pwallet)) {
                LogPrintf("UTUMMiner(): Signing new block failed \n");
                continue;
            }
        ...
        LogPrintf("Running UTUMMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void static ThreadBitcoinMiner(void* parg)
{
    ...
    try {
        ...
        LogPrintf("ThreadUTUMMiner() exception");
    } catch (...) {
        LogPrintf("ThreadUTUMMiner() exception");
    }

    LogPrintf("ThreadUTUMMiner exiting\n");
}
