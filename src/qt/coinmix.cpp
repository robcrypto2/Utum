...
class TxViewDelegate2 : public QAbstractItemDelegate
{
    Q_OBJECT
public:
    TxViewDelegate2() : QAbstractItemDelegate(), unit(BitcoinUnits::UTUM)
    {
    }
...
void Coinmix::setWalletModel(WalletModel* model)
{
    ...
    // update the display unit, to not use the default ("UTUM")
    //updateDisplayUnit();
}
...
void Coinmix::updateDarksendProgress()
{
    ...
    QString strAnonymizeUTUMAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizeUTUMAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if (currentBalance == 0) {
        ...
        strAnonymizeUTUMAmount = strAnonymizeUTUMAmount.remove(strAnonymizeUTUMAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeUTUMAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    ...
    if (nMaxToAnonymize > nAnonymizeUTUMAmount * COIN) nMaxToAnonymize = nAnonymizeUTUMAmount * COIN;

    if (nMaxToAnonymize == 0) return;

    if (nMaxToAnonymize >= nAnonymizeUTUMAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                              .arg(strAnonymizeUTUMAmount));
        strAnonymizeUTUMAmount = strAnonymizeUTUMAmount.remove(strAnonymizeUTUMAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeUTUMAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    } else {
